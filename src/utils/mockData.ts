...
import { User, Transaction, SavingGoal, RecurringTransaction, SmartAlert } from '../types';
import { subDays, subMonths, addDays, format } from 'date-fns';

// Configure faker to Spanish
faker.locale = 'es';
...
export const mockTransactions: Transaction[] = Array.from({ length: 50 }, () => {
  const type = faker.helpers.arrayElement(['income', 'expense', 'saving']);
  ...
  return {
    ...
    status: faker.helpers.arrayElement(['approved', 'pending', 'approved', 'approved']),
  };
});
...
export const mockRecurringTransactions: RecurringTransaction[] = [
  {
    ...
    next_due_date: format(new Date(new Date().getFullYear(), new Date().getMonth() + 1, 15), 'yyyy-MM-dd')
  },
  {
    id: faker.string.uuid(),
    user_id: mockUsers[0].id,
    ...
    next_due_date: format(new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1), 'yyyy-MM-dd')
  }
];
